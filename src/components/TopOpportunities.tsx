            ...
            <tr key={opportunity.symbol} className="hover:bg-purple-900/20 transition-colors">
              ...
              <td className="px-4 py-4 whitespace-nowrap">
                <div className="flex items-center justify-center">
                  {(() => {
                    const style = getSignalBadgeStyle(opportunity.signalType, opportunity.signalStrength);
                    return (
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${style.bg} ${style.text} border ${style.border}`}>
                        ...
                        {opportunity.signalStrength ? (
                          <span className="ml-1 text-xs opacity-70">({opportunity.signalStrength})</span>
                        ) : null}
                      </span>
                    );
                  })()}
                </div>
                {opportunity.trendDirection !== 'NEUTRAL' && (
                  <div className="text-xs text-center mt-1 text-gray-500">
                    Trend: {opportunity.trendDirection === 'UP' ? '↗️ Upward' : '↘️ Downward'}
                  </div>
                )}
              </td>
            </tr>
